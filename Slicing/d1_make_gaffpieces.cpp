...
int main()
{
    ...
    */
    ...
    ifstream nl("fix_d1_namelist.txt");
...
// for outputs
    ...
    ofstream output ("fix_d1_fc_slice.txt"); //output!
    for(int i=0;i<36;i++)
    {
        ...
        for(int j=0;j<4;j++)
        {
           // output<<"X -ce-ce-X    4    "<<fc[i][j]/2<<"       180.000           2.000"<<"      a1/2"<<endl;

            if(a[i][j]>0)
            output<<"X -ce-cd-X    4"<<std::setw(9)<<a[i][j]/(double(2))<<std::setw(14)<<b[i][j]<<std::setw(16)<<2*double(j+1)<<endl;
            
            else
            output<<"X -ce-cd-X    4"<<std::setw(9)<<-a[i][j]/(double(2))<<std::setw(14)<<b[i][j]+double(180)<<std::setw(16)<<2*double(j+1)<<endl;
